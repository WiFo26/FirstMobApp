...
class ScheduleDetailDialogFragment : DialogFragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        itemScheduleDialogTitle.text = conference.title
        val pattern = "dd/MM/yyyy hh:mm aa"
        val simpleDF = SimpleDateFormat(pattern)
        val date = simpleDF.format(conference.datetime)
        itemScheduleDialogHour.text = date
        itemScheduleDialogSpeaker.text = conference.speaker
        itemScheduleDialogTag.text = conference.tag
        itemScheduleDialogDesc.text = conference.description
    }
